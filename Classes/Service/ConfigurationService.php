...
class ConfigurationService implements SingletonInterface
{
        ...
        foreach ($this->packageManager->getActivePackages() as $package) {
            ...
            if (true === is_dir($packageFormConfigurationsPath)) {
                foreach (glob($packageFormConfigurationsPath . '/*.yaml') as $formConfigurationFile) {
                    ...
                    } catch (Configuration\Exception $exception) {
    ...
    public function registerBackendContentElementWizards(): void
    {
        \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::addPageTSConfig('
            mod.wizards.newContentElement.wizardItems.forms {
              show := addToList(mailer_form)
              elements {
                mailer_form {
                  iconIdentifier = mailer-plugin
                  title = LLL:EXT:mailer/Resources/Private/Language/locallang.xlf:plugin.form.title
                  description = LLL:EXT:mailer/Resources/Private/Language/locallang.xlf:plugin.form.description
        ...
        // TODO: get all configurations with wizard title and add
        $formIdentifier = 'mailer/Example';
        $formIdentifierHash = hash('sha256', $formIdentifier);
        $formWizardTitle = 'Testformular';
        $formWizardDescription = 'Testbeschreibung';
        $formWizardIconIdentifier = 'content-form';
            ...
            mod.wizards.newContentElement.wizardItems.forms {
              ...
              elements {
                mailer_form_' . $formIdentifierHash . ' {
                  iconIdentifier = ' . $formWizardIconIdentifier . '
                  title = ' . $formWizardTitle . '
                  description = ' . $formWizardDescription . '
                  tt_content_defValues {
                    CType = mailer_form
                    mailer_form = ' . $formIdentifier . '
                    sometestfield = irgendwas test
                  }
                }
              }
            }
    ...
    protected function processConfiguration(array &$configuration): void
    {
        ...
        array_walk_recursive($configuration, function (&$item, &$key) use ($configuration) {
            ...
            if (true === is_string($key)) {
                $key = TranslationUtility::translateLLL($key);
            }
        });
    }
    ...
    protected function loadValidators(array $validatorConfigurations): array
    {
        ...
        foreach ($validatorConfigurations as $validatorConfiguration) {
            ...
            if (false === is_array($validatorConfiguration['options'])) {
                $validatorConfiguration['options'] = [];
            }

            if (false === is_string($validatorConfiguration['message'])) {
    ...
    protected function loadFields(array $fieldsConfiguration): array
    {
        ...
        foreach ($fieldsConfiguration as $fieldName => $fieldConfiguration) {
            ...
            if (true === is_array($fieldConfiguration['typeConverter'])) {
